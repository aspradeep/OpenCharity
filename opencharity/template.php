...
// Add Zen Tabs styles
if (theme_get_setting('opencharity_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'opencharity') . '/css/tabs.css');
}
...
function opencharity_preprocess_html(&$vars) {
  if (!module_exists('conditional_styles')) {
    opencharity_add_conditional_styles();
  }
...
function opencharity_preprocess_page(&$vars, $hook) {
...
function opencharity_preprocess_block(&$vars, $hook) {
  // Add a striping class.
...
function opencharity_breadcrumb($variables) {
  ...
  $show_breadcrumb = theme_get_setting('opencharity_breadcrumb');
  if ($show_breadcrumb == 'yes' || ($show_breadcrumb == 'admin' && arg(0) == 'admin')) {



    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('opencharity_breadcrumb_home');
    if (!$show_breadcrumb_home) {
      array_shift($breadcrumb);
    }
    // Return the breadcrumb with separators.
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('opencharity_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('opencharity_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('opencharity_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function opencharity_add_conditional_styles() {
 ...
 */
function opencharity_preprocess_menu_local_task(&$variables) {
...
function opencharity_menu_local_tasks(&$variables) {
  $output = '';
